...
class ROSExplorerPub(Node):
    ...
    def pub_ref(self,joint_ref=[None]*7):
        msg = DynamicJointState()
        i = 0
        if len(joint_ref) != 7:
            joint_ref = random.sample(range(-180,180),7)
        for i in range(0,7):
            # Publish !
...
class PyVESCExplorerBridge(Node):
    ...
    def __init__(self,can_port='vcan0',can_id=40):
        ...
        self.declare_parameter('vesc_joints_can_ids',[41,42,43,44,45,46,46])
        can_ids = self.get_parameter('vesc_joints_can_ids').get_parameter_value().integer_array_value
        self.declare_parameter('vesc_joint_names',['joint_1','joint_2','joint_3','joint_4','joint_5','joint_6','joint_7'])
        joint_names = self.get_parameter('vesc_joint_names').get_parameter_value().string_array_value
        self.declare_parameter('vesc_joint_types',['motor','motor','motor','motor','motor','motor','servo'])
