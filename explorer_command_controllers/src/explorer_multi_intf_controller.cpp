...
controller_interface::CallbackReturn MultiIntfController::on_init()
{
  ...
  printJoint_srv =
    get_node()->create_service<std_srvs::srv::Empty>("~/printJoints", [this]
  ...
  const rclcpp_lifecycle::State&)
{
  ...
  //{
  //  RCLCPP_ERROR(get_node()->get_logger(), "on_configure:: Already configured");
  //  return controller_interface::CallbackReturn::ERROR;
  //}
  ...
  refs_subscriber_ = get_node()->create_subscription<CmdType>("~/refs", subscribers_qos,
    [this](const CmdType::SharedPtr msg) 
    { 
      refs_rt_b_.writeFromNonRT(msg); 
    });

  return controller_interface::CallbackReturn::SUCCESS;
}

controller_interface::InterfaceConfiguration MultiIntfController::command_interface_configuration() const
{
  const auto& log = get_node()->get_logger();
...
controller_interface::InterfaceConfiguration MultiIntfController::state_interface_configuration() const
{
  const auto& log = get_node()->get_logger();
...
void MultiIntfController::printJoints()
{
  const auto& log = get_node()->get_logger();
...
std::vector<hardware_interface::CommandInterface> MultiIntfController::on_export_reference_interfaces()
{
  const auto& log = get_node()->get_logger();
    ...
    const rclcpp::Time &, const rclcpp::Duration &)
{
  ...
  {
    if ((*refs)->data.size() != _joints.size())
    {
      RCLCPP_ERROR_THROTTLE(log, clock, 1000,
        "refs size (%zu) does not match number of joints (%zu)",
        (*refs)->data.size(), _joints.size());
      return controller_interface::return_type::ERROR;
    }
      ...
      if(j.settings.enable_limits)
      {
        ...
        if(v > j.settings.cmd_max)
        {
          //RCLCPP_ERROR_THROTTLE(log, clock, 100,\
            "[%s] Ref is above cmd_max: %f > %f. Capping to cmd_max.",j.name.c_str(), v, j.settings.cmd_max);
          v = j.cmd_v = j.settings.cmd_max;
        }
        else if(v < j.settings.cmd_min)
        {
          //RCLCPP_ERROR_THROTTLE(log, clock, 100,\
            "[%s] Ref is below cmd_min %f < %f. Capping to cmd_min.",j.name.c_str(), v, j.settings.cmd_min);
        ...
        const auto step_max = j.settings.step_max;
        {
          ...
          if(std::abs(diff) > step_max)
          {
            ...
            //RCLCPP_ERROR_THROTTLE(log, clock, 100,\
              "[%s] CMD step is over step_max: %f > %f. Capping to step_max. cmd_p: %f cmd_v: %f, new_cmd_v %f",j.name.c_str(), std::abs(diff), j.settings.step_max, j.cmd_v_p, j.cmd_v, j.cmd_v_p + ndiff);
            v = j.cmd_v_p + ndiff;
          }
        }
      }
