...
namespace space_control
{
    ...
    trajectory_msgs::msg::JointTrajectory TrajectoryManager::getTrajectory() {
        ...
        traj_point_target.positions.push_back(gripper); // keep gripper position unchanged
    ...
    void TrajectoryManager::updateStatusString()
    {
        if(!trajectory_completed_){
            if (pointAlmostEqual(q_current_, init_points_[0])) {
                status = "retracted";
            }
            else if (pointAlmostEqual(q_current_, init_points_.back())) {
                status = "ready";
            }
            else {
                status = "in progress";
