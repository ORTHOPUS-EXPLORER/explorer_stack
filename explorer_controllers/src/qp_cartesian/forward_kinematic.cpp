...
: n_(n), joint_number_(joint_number), x_current_(), q_current_(joint_number)
{
...
void ForwardKinematic::reset() { init_flag_ = true; }
...
void ForwardKinematic::setQCurrent(const JointPosition& q_current) { q_current_ = q_current; }

void ForwardKinematic::getXCurrent(SpacePosition& x_current) { x_current = x_current_; }
}  // namespace space_control
