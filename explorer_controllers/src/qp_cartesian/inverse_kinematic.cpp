  ...
  prev_dq_computed_(joint_number)
{
  ...
  if (!n_->get_parameter("position_control_frame", position_frame_str))
  {
    ...
    RCLCPP_INFO(
      n_->get_logger(), "Using default position_control_frame: %s", position_frame_str.c_str());
  }
  if (!n_->get_parameter("orientation_control_frame", orientation_frame_str))
  {
    orientation_frame_str = "World";  // Default to World frame
    RCLCPP_INFO(
      n_->get_logger(), "Using default orientation_control_frame: %s",
      orientation_frame_str.c_str());
  }
  ...
  RCLCPP_INFO(
    n_->get_logger(), "Control frames - Position: %s, Orientation: %s", position_frame_str.c_str(),
    orientation_frame_str.c_str());
  ...
  if (!n_->get_parameter("j5_alignment_threshold", j5_alignment_threshold_))
  {
    ...
    RCLCPP_INFO(
      n_->get_logger(), "Using default j5_alignment_threshold: %.3f rad", j5_alignment_threshold_);
  }

  // Movement detection threshold parameter for J4-J6 centering
  if (!n_->get_parameter(
        "movement_detection_threshold_centering", movement_detection_threshold_centering_))
  {
    movement_detection_threshold_centering_ =
      1e-6;  // Default: very small threshold for any intentional movement
    RCLCPP_INFO(
      n_->get_logger(), "Using default movement_detection_threshold_centering: %.2e",
      movement_detection_threshold_centering_);
  }
  ...
  {
    RCLCPP_INFO(
      n_->get_logger(), "Loaded enable_adaptive_snap: %s",
  ...
  if (!n_->get_parameter("adaptive_snap_threshold_pos", adaptive_snap_threshold_pos_))
  {
    ...
    RCLCPP_INFO(
      n_->get_logger(), "Using default adaptive_snap_threshold_pos: %.3f m/s",
      adaptive_snap_threshold_pos_);
  }
  else
  {
    RCLCPP_INFO(
      n_->get_logger(), "Loaded adaptive_snap_threshold_pos: %.3f m/s",
      adaptive_snap_threshold_pos_);
  }
  ...
  if (!n_->get_parameter("adaptive_snap_threshold_or", adaptive_snap_threshold_or_))
  {
    ...
    RCLCPP_INFO(
      n_->get_logger(), "Using default adaptive_snap_threshold_or: %.3f",
      adaptive_snap_threshold_or_);
  }
  else
  {
    RCLCPP_INFO(
      n_->get_logger(), "Loaded adaptive_snap_threshold_or: %.3f", adaptive_snap_threshold_or_);
  }
  ...
  if (!n_->get_parameter("adaptive_snap_cycles", adaptive_snap_cycles_required_))
  {
    ...
    RCLCPP_INFO(
      n_->get_logger(), "Using default adaptive_snap_cycles: %d cycles",
      adaptive_snap_cycles_required_);
  }
  else
  {
    RCLCPP_INFO(
      n_->get_logger(), "Loaded adaptive_snap_cycles: %d cycles", adaptive_snap_cycles_required_);
  }
  ...
  if (!n_->get_parameter("gamma_suppression_cycles", gamma_suppression_cycles_))
  {
    ...
    RCLCPP_INFO(
      n_->get_logger(), "Using default gamma_suppression_cycles: %d cycles",
      gamma_suppression_cycles_);
  }
  else
  {
    RCLCPP_INFO(
      n_->get_logger(), "Loaded gamma_suppression_cycles: %d cycles", gamma_suppression_cycles_);
  }
  ...
  if (!n_->get_parameter("snap_input_threshold", snap_input_threshold_))
  {
    ...
    RCLCPP_INFO(
      n_->get_logger(), "Using default snap_input_threshold: %.2e", snap_input_threshold_);
  }
  else
  {
    RCLCPP_INFO(n_->get_logger(), "Loaded snap_input_threshold: %.2e", snap_input_threshold_);
  }
  ...
  cb_handle_alpha_weight =
    param_subscriber_->add_parameter_callback("alpha_weight", callback_alpha_weight);
  cb_handle_alpha_multiplier =
    param_subscriber_->add_parameter_callback("alpha_multiplier", callback_alpha_multiplier);
  cb_handle_beta_weight =
    param_subscriber_->add_parameter_callback("beta_weight", callback_beta_weight);
  cb_handle_beta_multiplier =
    param_subscriber_->add_parameter_callback("beta_multiplier", callback_beta_multiplier);
  cb_handle_gamma_weight =
    param_subscriber_->add_parameter_callback("gamma_weight", callback_gamma_weight);
  cb_handle_gamma_multiplier =
    param_subscriber_->add_parameter_callback("gamma_multiplier", callback_gamma_multiplier);
  cb_handle_joint_centering_weight = param_subscriber_->add_parameter_callback(
    "joint_centering_weight", callback_joint_centering_weight);
  cb_handle_joint_centering_multiplier = param_subscriber_->add_parameter_callback(
    "joint_centering_multiplier", callback_joint_centering_multiplier);
  cb_handle_position_control_frame = param_subscriber_->add_parameter_callback(
    "position_control_frame", callback_position_control_frame);
  cb_handle_orientation_control_frame = param_subscriber_->add_parameter_callback(
    "orientation_control_frame", callback_orientation_control_frame);
...
void InverseKinematic::setAlphaWeight_(
  const std::vector<double>& alpha_weight, const int alpha_multiplier)
{
...
void InverseKinematic::setBetaWeight_(
  const std::vector<double>& beta_weight, const int beta_multiplier)
{
...
void InverseKinematic::setGammaWeight_(
  const std::vector<double>& gamma_weight, const int gamma_multiplier)
{
  ...
  RCLCPP_DEBUG_STREAM(
    n_->get_logger(), "Set gamma position weight to : \n"
                        << gamma_pos_weight_ << "\n");
  RCLCPP_DEBUG_STREAM(
    n_->get_logger(), "Set gamma orientation weight to : \n"
                        << gamma_or_weight_ << "\n");
}

void InverseKinematic::setJointCenteringWeight_(
  const std::vector<double>& joint_centering_weight, const int joint_centering_multiplier)
{
  ...
  RCLCPP_DEBUG_STREAM(
    n_->get_logger(), "Set joint centering weight to : \n"
...
void InverseKinematic::setQCurrent(const JointPosition& q_current) { q_current_ = q_current; }
...
void InverseKinematic::resolveInverseKinematic(
  ...
  bool path_tracking_mode, bool wheelchair)
{
  ...
  {
    getJacobian_(
      kinematic_state_, joint_model_group_, kinematic_state_->getLinkModel(jacobian_frame),
      reference_point_position, jacobian, true);

    if (jacobian_frame != end_effector_link_)
    {
      RCLCPP_DEBUG_THROTTLE(
        n_->get_logger(), *n_->get_clock(), 10000, "Using Jacobian computed for frame: %s",
        jacobian_frame.c_str());
    }
  }
  catch (const std::exception& e)
  {
    ...
    RCLCPP_WARN_THROTTLE(
      n_->get_logger(), *n_->get_clock(), 10000, "Frame '%s' not found, falling back to %s: %s",
      jacobian_frame.c_str(), end_effector_link_.c_str(), e.what());
    getJacobian_(
      kinematic_state_, joint_model_group_, kinematic_state_->getLinkModel(end_effector_link_),
      reference_point_position, jacobian, true);
  }
    ...
    else
    {
      if (flag_pos_save[i] == true)
      {
        ...
        if (adaptive_snap_triggered_pos_)
        {
          ...
          RCLCPP_WARN(
            n_->get_logger(),
            "Joystick released after adaptive snap - updating pos AND orientation snap, activating "
            "BOTH gamma suppressions for %d cycles",
            gamma_suppression_cycles_);
        }
      }
    }
  ...
  double x_snap_vect[] = {pos_snap_[0], pos_snap_[1], pos_snap_[2], r_snap_.w(),
                          r_snap_.x(),  r_snap_.y(),  r_snap_.z()};
  ...
  VectorXd g;
  computeObjectives_(
    hessian, g, dx_desired_in_frame, x_des_objective, jacobian, path_tracking_mode);
  ...
  if (qp_init_required_)
  {
    ...
    qp_return = QP_->init(
      hessian.data(), g.data(), A.data(), dq_lower_limit_.data(), dq_upper_limit_.data(),
      lbA.data(), ubA.data(), nWSR, 0);
    qp_init_required_ = false;
  }
  else
  {
    qp_return = QP_->hotstart(
      hessian.data(), g.data(), A.data(), dq_lower_limit_.data(), dq_upper_limit_.data(),
      lbA.data(), ubA.data(), nWSR, 0);
  }
  ...
  else
  {
    ...
    {
      dq_computed[i] =
        prev_dq_computed_[i] * 0.5;  // Reduce previous velocity by half as conservative fallback
    }
  }

  /* Only reset on serious errors, not on max iterations reached */
  if (
    qp_return != qpOASES::SUCCESSFUL_RETURN && qp_return != qpOASES::RET_MAX_NWSR_REACHED &&
  ...
  if (enable_adaptive_snap_)
  {
    ...
    bool user_commanding_pos =
      (dx_pos_desired.norm() > 1e-3);  // 1mm/s threshold for "user is commanding"
    ...
    RCLCPP_WARN_THROTTLE(
      ...
      dx_pos_desired.norm(), dx_or_desired.norm(), pos_error, or_error, user_commanding_pos,
      user_commanding_or, pos_sacrifice, or_sacrifice);
    ...
    if (pos_sacrifice)
    {
      ...
      RCLCPP_INFO_THROTTLE(
        ...
        adaptive_snap_counter_pos_, adaptive_snap_cycles_required_, pos_error,
        adaptive_snap_threshold_pos_);
    }
    else
    {
    ...
    if (or_sacrifice)
    {
      ...
      RCLCPP_INFO_THROTTLE(
        ...
        adaptive_snap_counter_or_, adaptive_snap_cycles_required_, or_error,
        adaptive_snap_threshold_or_);
    }
    else
    {
    ...
    if (adaptive_snap_counter_pos_ >= adaptive_snap_cycles_required_)
    {
      ...
      RCLCPP_WARN(
        n_->get_logger(),
        "ADAPTIVE SNAP TRIGGERED: Position sacrifice sustained for %d cycles - will update pos AND "
        "orientation snap on joystick release",
        adaptive_snap_cycles_required_);
    }
    ...
    if (adaptive_snap_counter_or_ >= adaptive_snap_cycles_required_)
    {
      ...
      RCLCPP_WARN(
        n_->get_logger(),
        "ADAPTIVE SNAP TRIGGERED: Orientation sacrifice sustained for %d cycles - will update pos "
        "AND orientation snap on joystick release",
        adaptive_snap_cycles_required_);
    }
  }
}

void InverseKinematic::computeVelocityInFrame_(
  SpaceVelocity& dx_desired_in_frame, const SpaceVelocity& dx_desired, const Matrix3d& R_0to1)
{
  ...
  Quaterniond r_half_omega(
    0.0, 0.5 * dx_desired.orientation.x(), 0.5 * dx_desired.orientation.y(),
...
void InverseKinematic::computeObjectives_(
  ...
  const MatrixXd& jacobian, bool path_tracking)
{
  ...
  if (enable_adaptive_snap_)
  {
    ...
    if (gamma_suppression_counter_pos_ > 0)
    {
      if (!user_commanding)
      {
        ...
        RCLCPP_INFO_THROTTLE(
          n_->get_logger(), *n_->get_clock(), 200,
        ...
        if (gamma_suppression_counter_pos_ == 0)
        {
          ...
          RCLCPP_WARN(
            n_->get_logger(),
            "Position gamma suppression ended - snap updated to current position, resuming normal "
            "control");
        }
      }
      else
    ...
    if (gamma_suppression_counter_or_ > 0)
    {
      if (!user_commanding)
      {
        ...
        if (gamma_suppression_counter_or_ == 0)
        {
          ...
          RCLCPP_INFO(
            n_->get_logger(),
            "Orientation gamma suppression ended - snap updated to current orientation, resuming "
            "normal control");
        }
      }
    }
  ...
  hessian = jacobian.transpose() * alpha_weight_ * dx_controlled_mat * jacobian +
            jacob_pos.transpose() * gamma_pos_weight_ * pos_controlled_mat * jacob_pos *
              sampling_period_ * sampling_period_ +
            jacob_or.transpose() * Rdes_conj.transpose() * gamma_or_weight_ * or_controlled_mat *
              Rdes_conj * jacob_or * sampling_period_ * sampling_period_ +
            beta_weight_ +
  ...
  if (centering_scale > 0.0)
  {
    ...
    for (int i = 0; i < joint_number_; i++)
    {
      if (i != 3 && i != 5)
      {  // Not J4 or J6
        smart_centering_gradient(i) =
          joint_centering_weight_(i, i) * q_current_eigen(i) * centering_scale;
      }
    }
  ...
  g = -jacobian.transpose() * alpha_weight_ * dx_controlled_mat * dx_des.getRawVector() +
      jacob_pos.transpose() * gamma_pos_weight_ * pos_controlled_mat *
        (x_current_.getPosition() - x_des.getPosition()) * sampling_period_ +
      jacob_or.transpose() * Rdes_conj.transpose() * gamma_or_weight_ * or_controlled_mat *
        Rdes_conj * x_current_.getOrientation().toVector() * sampling_period_ +
...
void InverseKinematic::computeConstraints_(
  MatrixXdRowMaj& A, std::vector<double>& lbA, std::vector<double>& ubA,
  [[maybe_unused]] const MatrixXd& jacobian, bool wheelchair)
{
  A.resize(joint_number_, joint_number_);

  A.topLeftCorner(joint_number_, joint_number_) =
    MatrixXd::Identity(joint_number_, joint_number_) * sampling_period_;
...
bool InverseKinematic::getJacobian_(
  const moveit::core::RobotStatePtr kinematic_state, const moveit::core::JointModelGroup* group,
  const moveit::core::LinkModel* link, const Vector3d& reference_point_position, MatrixXd& jacobian,
  bool use_quaternion_representation)
{
  ...
  {
    RCLCPP_ERROR(
      n_->get_logger(), "The group '%s' is not a chain. Cannot compute Jacobian.",
      group->getName().c_str());
    return false;
  }
  ...
  const moveit::core::JointModel* root_joint_model =
    group->getJointModels()[0];  // group->getJointRoots()[0];
  ...
  Isometry3d reference_transform =
    root_link_model ? kinematic_state->getGlobalLinkTransform(root_link_model).inverse()
                    : Isometry3d::Identity();
  ...
  // getGlobalLinkTransform() returns a valid isometry by contract
  Isometry3d link_transform =
    reference_transform * kinematic_state->getGlobalLinkTransform(link);  // valid isometry
  ...
  */
  ...
  while (link)
  {
    ...
    if (pjm->getVariableCount() > 0)
    {
      ...
      {
        joint_axis = joint_transform.linear() *
                     static_cast<const moveit::core::RevoluteJointModel*>(pjm)->getAxis();
      ...
      {
        joint_axis = joint_transform.linear() *
                     static_cast<const moveit::core::PrismaticJointModel*>(pjm)->getAxis();
  ...
  if (use_quaternion_representation)
  {
    ...
    else
    {
      ...
      double dot_product =
        conv_quat.w() * jacobian_quat_prev_.w() + conv_quat.x() * jacobian_quat_prev_.x() +
        conv_quat.y() * jacobian_quat_prev_.y() + conv_quat.z() * jacobian_quat_prev_.z();
    ...
    jacobian.block(3, 0, 4, columns) =
      0.5 * xR_(conv_quat).block(0, 1, 4, 3) * jacobian.block(3, 0, 3, columns);
  }
  return true;
}

void InverseKinematic::checkConstraintsDebug_(
